...
The application creates a default administrator user during initialization.
...
After logging in, use the returned session token as a Bearer token for authenticated administrator endpoints.

> **Security note:** These credentials are intended for local development/testing only. Change or remove the default credentials before deploying the application to a production environment.
